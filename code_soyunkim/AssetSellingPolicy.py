...
class AssetSellingPolicy():
    ...
    def track_policy(self, state, info_tuple):
        ...
        - 기존 track의 파라미터 튜플 형식은 그대로 허용:
            (theta, gamma, prev_price, prev_price2)  또는  (theta, prev_price, prev_price2)
        => gamma(두 번째 요소)는 이번 문제(2.9) 기준으로 '무시'한다.
        - 의사결정 규칙을 |p_t - pbar_t| > theta 로 변경.
    ...
    def old_track_policy(self, state, info_tuple):
        """
